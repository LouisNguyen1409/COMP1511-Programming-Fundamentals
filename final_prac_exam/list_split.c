...
struct split_list *split(struct node *head) {
    ...
    struct split_list *return_struct = malloc(sizeof(struct split_list));
    return_struct->before = NULL;
    return_struct->after = NULL;

    while (curr != NULL) {
        if (curr->data == 0 && pre != NULL) {
            return_struct->after = curr;
            pre->next = NULL;
            return_struct->before = head;
            return return_struct;
        } else if (curr->data == 0 && pre == NULL) {
            return_struct->after = head;
            return return_struct;
        }
        pre = curr;
        curr = curr->next;
